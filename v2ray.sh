...
KCP="false"
create_v2ray_files() {
    if [[ ! -e "v2ray.pem" && ! -e "v2ray.key" ]]; then
        # Ask for the content of v2ray.pem
        echo "Please enter the content for 'v2ray.pem' (Press Ctrl+D when done):"
        cat > v2ray.pem

        # Ask for the content of v2ray.key
        echo "Please enter the content for 'v2ray.key' (Press Ctrl+D when done):"
        cat > v2ray.key

        echo "Files 'v2ray.pem' and 'v2ray.key' have been created."
    elif [[ ! -e "v2ray.pem" ]]; then
        echo "File 'v2ray.pem' is missing."
        # Ask for the content of v2ray.pem
        echo "Please enter the content for 'v2ray.pem' (Press Ctrl+D when done):"
        cat > v2ray.pem

        echo "File 'v2ray.pem' has been created."
    elif [[ ! -e "v2ray.key" ]]; then
        echo "File 'v2ray.key' is missing."
        # Ask for the content of v2ray.key
        echo "Please enter the content for 'v2ray.key' (Press Ctrl+D when done):"
        cat > v2ray.key

        echo "File 'v2ray.key' has been created."
    else
        echo "Both 'v2ray.pem' and 'v2ray.key' files already exist."
    fi
}
checkSystem() {
    create_v2ray_files
...
installV2ray() {
    rm -rf /tmp/v2ray
    mkdir -p /tmp/v2ray
    DOWNLOAD_LINK="https://github.com/v2fly/v2ray-core/releases/download/v4.45.2/v2ray-linux-$(archAffix).zip"
...
showInfo() {
    ...
    json=$(cat /etc/v2ray/config.json)
    urlencode() {
    local string="$1"
    local length="${#string}"
    ...
    # Extract the required values using jq
    protocol=$(echo "$json" | jq -r '.inbounds[0].protocol')
    id=$(echo "$json" | jq -r '.inbounds[0].settings.clients[0].id')
    path=$(echo "$json" | jq -r '.inbounds[0].streamSettings.wsSettings.path')
    encoded_string=$(urlencode "$path")
    security=$(echo "$json" | jq -r '.inbounds[0].streamSettings.security')
